...
// .wrangler/tmp/bundle-hvoYDT/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-hvoYDT/middleware-insertion-facade.js
init_modules_watch_stub();
...
async function fetchYoutubeSubtitle(videoId) {
  try {
    ...
    const lines = xml.replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").split("\n").map((l) => l.trim()).filter(Boolean);
    return lines.join(" ");
  } catch {
    return null;
  }
}
__name(fetchYoutubeSubtitle, "fetchYoutubeSubtitle");
...
// node_modules/.pnpm/@google+genai@2.8.0/node_modules/@google/genai/dist/web/index.mjs
init_modules_watch_stub();
...
// src/lib/gemini.ts
var ALLOWED_MODELS = ["gemini-2.5-pro", "gemini-2.5-flash"];
function validateModel(model) {
  if (ALLOWED_MODELS.includes(model)) return model;
  return "gemini-2.5-flash";
}
__name(validateModel, "validateModel");
async function* streamGenerate(apiKey, model, prompt) {
...
async function generateStructured(apiKey, model, prompt) {
  const ai = new GoogleGenAI({ apiKey });
  const response = await ai.models.generateContent({
    model,
    contents: prompt,
...
__name(generateStructured, "generateStructured");
...
// src/lib/contextStore.ts
async function saveContext(kv, context, ttl) {
  const articleId = nanoid();
  const effectiveTtl = Math.max(ttl, 60);
  await kv.put(articleId, JSON.stringify(context), { expirationTtl: effectiveTtl });
  return articleId;
}
__name(saveContext, "saveContext");
async function getContext(kv, articleId) {
  return kv.get(articleId, { type: "json" });
}
...
__name(toSSE, "toSSE");
function toSSEJSON(event, payload) {
  return toSSE(event, JSON.stringify(payload));
}
__name(toSSEJSON, "toSSEJSON");

// src/prompts.ts
init_modules_watch_stub();
function buildArticlePrompt(subtitles, userRequirements) {
...
function build5W1HPrompt(subtitles, fullArticle, chapterTitle, chapterHtml) {
  return `\u3010\u7CFB\u7EDF\u89D2\u8272\u3011
...
__name(build5W1HPrompt, "build5W1HPrompt");

// src/handlers/generate.ts
var CSS_BLOCK_RE = /<style[^>]*>[\s\S]*?<\/style>/gi;
function stripCSS(html) {
  return html.replace(CSS_BLOCK_RE, "");
}
__name(stripCSS, "stripCSS");
...
async function processStream(writer, encoder, params, env) {
...
function assignChapterHtml(chapters, html) {
...
__name(assignChapterHtml, "assignChapterHtml");

// src/handlers/fiveWH.ts
init_modules_watch_stub();
async function handleFiveWH(request, env) {
  const body = await request.json();
...
__name(handleFiveWH, "handleFiveWH");
